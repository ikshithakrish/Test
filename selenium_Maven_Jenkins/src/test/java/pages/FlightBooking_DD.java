...
public class FlightBooking_DD {
	public static final Logger LOGGER = LogManager.getLogger(FlightBooking_DD.class);
...
//				if(HtmlCreator.copyFileContent(HtmlCreator.BUFFERED_WRITER, reportHeaderFile)) {
					...
					if(ALL_ROW_DATA.length > 0){  // if ALL_ROW_DATA.length > 0   - BEGIN
						...
						for(ROW_NUMBER = 0; ROW_NUMBER< ALL_ROW_DATA.length; ROW_NUMBER++){  // for ROW_NUMBER - BEGIN 
							...
							boolean result = SeleniumActions_COPY.optionType_TEMP(SINGLE_ROW_DATA, inputData);
							HtmlCreator.stringAppender(bw, "<tr>");
							for(int i=0; i< SINGLE_ROW_DATA.size(); i++ ) {
								if( (i == 5) && (SeleniumActions_COPY.PARAM_VALUE != null) ) {
									//SINGLE_ROW_DATA.add(i, SeleniumActions_COPY.PARAM_VALUE);
									SINGLE_ROW_DATA.remove(i); // remove existing value(PARAMETER_*)
									SINGLE_ROW_DATA.add(i, SeleniumActions_COPY.PARAM_VALUE);  // add new value in PARAMETER_* position
									HtmlCreator.stringAppender(bw, "<td style=\"border-style: solid; border-width: 1px;\">" + SINGLE_ROW_DATA.get(i) + "</td>");	
								}
								else if( (i == 5) && (SeleniumActions_COPY.ACTION_TYPE.equals(Constants.CAPTURE_SCREEN)) ) {
									//SINGLE_ROW_DATA.add(i, SeleniumActions_COPY.PARAM_VALUE);
									SINGLE_ROW_DATA.remove(i); // remove existing value(PARAMETER_*)
									SINGLE_ROW_DATA.add(i, SeleniumActions_COPY.SCREENSHOT_HREF);  // add new value in PARAMETER_* position
